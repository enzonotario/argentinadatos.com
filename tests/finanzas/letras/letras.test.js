  ...
  it(
    ...
    async () => {
      ...
      if (datos.length > 0) {
        for (const item of datos) {
          expect(item.ticker).toMatch(/^[ST]\d{2}[EFMAYLGJSOND]\d/)
          expect(aFechaIso(item.fechaVencimiento)).toMatch(
            /^\d{4}-\d{2}-\d{2}$/,
          )

          if (item.fechaEmision) {
            expect(aFechaIso(item.fechaEmision)).toMatch(/^\d{4}-\d{2}-\d{2}$/)
            expect(
              aFechaIso(item.fechaEmision) <= aFechaIso(item.fechaVencimiento),
            ).toBe(true)
          }
