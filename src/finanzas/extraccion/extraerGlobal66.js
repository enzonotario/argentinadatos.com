...
export async function extraerGlobal66CuentaRemunerada() {
  ...
  try {
    ...
    if (!datos || datos.tna === null || datos.tna === undefined) {
      logMensaje(log, 'Datos inválidos de Global66 API', {
        datos,
      })
      throw new Error('Error en la respuesta de Global66 API')
    }

    const tna = Number((Number(datos.tna) / 100).toFixed(4))
    const tea = Number(((1 + tna / 365) ** 365 - 1).toFixed(4))

    var fecha = format(new Date(), 'yyyy-MM-dd')
